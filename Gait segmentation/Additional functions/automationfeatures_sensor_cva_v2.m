function featureout = automationfeatures_sensor_cva_v2(datain_as,datain_us,sensorstr,template,dt,Hz,ahs,ato,uhs,uto)
    ...
    for a = 1 : size(data_all,2)	% Sensor axis
        ...
        if s <= min(NumStepsA,NumStepsU)            
            ...
            if strcmp(sensorstr,'jt')   % Joint symmetry measures
                ...
                feat(s,1).(['Feat26',Axesnames{a}]) = abs(MvalueA-MvalueU)/(0.5*(MvalueA+MvalueU));	% Feature 26: difference of the maximum values
                feat(s,1).(['Feat27',Axesnames{a}]) = abs(mvalueA-mvalueU)/(0.5*(mvalueA+mvalueU));	% Feature 27: difference of the minimum values
                feat(s,1).(['Feat28',Axesnames{a}]) = abs(rangeA-rangeU)/(0.5*(rangeA+rangeU));    	% Feature 28: difference of the rom
                feat(s,1).(['Feat29',Axesnames{a}]) = abs(meanA-meanU)/(0.5*(meanA+meanU));         % Feature 29: difference of the means
                feat(s,1).(['Feat30',Axesnames{a}]) = mean(abs(as_int_data-us_int_data)./...
