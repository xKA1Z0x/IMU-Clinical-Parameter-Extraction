function featureout = automationfeatures_sensor_hc(datain_right,datain_left,sensorstr,template,dt,Hz,rhs,rto,lhs,lto)
    ...
    for a = 1 : size(data_all,2)	% Sensor axis
        ...
        if s <= min(NumStepsL,NumStepsR)            
            ...
            if strcmp(sensorstr,'jt')   % Joint symmetry measures
                ...
                feat(s,1).(['Feat39',Axesnames{a}]) = abs(MvalueL-MvalueR)/(0.5*(MvalueL+MvalueR));	% Feature 39: difference of the maximum values
                feat(s,1).(['Feat40',Axesnames{a}]) = abs(mvalueL-mvalueR)/(0.5*(mvalueL+mvalueR));	% Feature 40: difference of the minimum values
                feat(s,1).(['Feat41',Axesnames{a}]) = abs(rangeL-rangeR)/(0.5*(rangeL+rangeR));    	% Feature 41: difference of the rom
                feat(s,1).(['Feat42',Axesnames{a}]) = abs(meanL-meanR)/(0.5*(meanL+meanR));         % Feature 42: difference of the means
                feat(s,1).(['Feat43',Axesnames{a}]) = mean(abs(left_int_data-right_int_data)./...
                    (0.5*(left_int_data+right_int_data)));     % Feature 43: mean point to point error
            end
        end
        ...
        switch a + 12*(strcmp(sensorstr,'ac')) + 24*(strcmp(sensorstr,'jt'))
            ...
            case 9  % Foot Gyroscope around y-axis
                ...
                feat(s,1).(['Feat29',featlabel{a}]) = topk_idx+TO_idx(s)-6;         % Feature 29: Toe-off Gyro timing
                ...
                feat(s,1).(['Feat31',featlabel{a}]) = fosw_idx+topk_idx+TO_idx(s)-5;% Feature 31: Max Gyro value in swing timing
            case 30 % Knee Flexion/Extension angle
                ...
                feat(s,1).(['Feat33',featlabel{a}]) = min(stanceknee);              % Feature 33: minimum value during swing
                feat(s,1).(['Feat34',featlabel{a}]) = range(stanceknee);            % Feature 34: rom during stance
                swingknee 	= data(TO_idx(s):end);
                feat(s,1).(['Feat35',featlabel{a}])	= max(swingknee);               % Feature 35; maximum value during stance
